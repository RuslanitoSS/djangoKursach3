...
class User(AbstractUser):
    profile_pic = models.ImageField(
        _("Фото профиля"), upload_to="user_profile_pics/", blank=True, null=True
    )
    description = models.TextField(_("Описание"), blank=True, null=True)
    login_code = models.CharField(
        _("Код входа"), max_length=255, blank=True, null=True
...
class UserPaymentMethod(models.Model):
    ...
    payment_type = models.CharField(
        _("Тип оплаты"), max_length=20, choices=PAYMENT_TYPE_CHOICES
    )
    provider_id = models.CharField(_("ID провайдера"), max_length=255)
    masked_card_number = models.CharField(
        _("Маска номера карты"), max_length=19, blank=True, null=True
    )
    card_brand = models.CharField(
        _("Платежная система"), max_length=50, blank=True, null=True
    )
...
class Franchise(models.Model):
    ...
    def get_chapter_overview(self):
        return list(
            self.chapters.values("id", "chapter_number", "franchise_relation").order_by(
                "chapter_number"
            )
        )
...
class Chapter(models.Model):
    ...
    franchise_relation = models.CharField(
        ...
        null=True,
        help_text=_(
            "Тип связи главы с франшизой (например, основная история, спин-офф и т.д.)"
        ),
    )
    ...
    poster_image = models.ImageField(
        _("Постер"), upload_to="chapter_posters/", blank=True, null=True
    )
    trailer_url = models.URLField(_("Ссылка на трейлер"), blank=True, null=True)

    genres = models.ManyToManyField(
        Genre, related_name="chapters", blank=True, verbose_name=_("Жанры")
    )
    people = models.ManyToManyField(
        "Person",
        through="ChapterPersonRole",
        related_name="chapters",
        blank=True,
    ...
    def get_actor_names(self):
        ...
        """
        return list(
            self.people.filter(chapter_roles__role="actor").values_list(
    ...
    def clean(self):
        ...
        for field_name, value in required_fields.items():
            if value is None:  # ✅ Правильная проверка на отсутствие значения
        ...
        if self.release_date and self.release_date > datetime.date.today():
            raise ValidationError(
                {"release_date": _("Дата релиза не может быть в будущем.")}
            )

        if self.age_rating and not (0 <= self.age_rating <= 21):
            raise ValidationError(
                {"age_rating": _("Возрастной рейтинг должен быть от 0 до 21.")}
            )

        if (
            Chapter.objects.exclude(id=self.id)
            .filter(title=self.title, release_date=self.release_date)
            .exists()
        ):
            raise ValidationError(
                _("Контент с таким названием и годом выпуска уже существует.")
            )
...
class Episode(models.Model):
    ...
    episode_number = models.PositiveIntegerField(
        _("Номер эпизода"), blank=True, null=True
    )
    title = models.CharField(_("Название"), max_length=255, blank=True, null=True)
    video_file = models.FileField(
        _("Видеофайл"), upload_to="episode_videos/", blank=True, null=True
    )
    duration = models.DurationField(_("Длительность"), blank=True, null=True)
    release_date = models.DateField(_("Дата выхода"), blank=True, null=True)
    thumbnail_img = models.ImageField(
        _("Превью"), upload_to="episode_thumbnail_imgs/", blank=True, null=True
    )
...
class ChapterPersonRole(models.Model):
    ...
    role = models.CharField(
        _("Роль"), max_length=20, choices=ROLE_CHOICES, blank=True, null=True
    )
    ...
    def clean(self):
        ...
        if self.text:
            ...
            for word in banned_words:
                if word in lowered:
                    raise ValidationError(
                        f"Комментарий содержит запрещённое слово: {word}"
                    )
...
class PlaylistChapter(models.Model):
    playlist = models.ForeignKey(
        Playlist,
        related_name="playlist_chapters",
        on_delete=models.CASCADE,
        null=True,
        blank=True,
        verbose_name=_("Плейлист"),
